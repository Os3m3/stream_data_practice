...
consumer.subscribe(['bad-drilling-data'])
...
cursor.execute("DROP TABLE IF EXISTS bad_drilling_data CASCADE;")
db_conn.commit()

cursor.execute(""" 
CREATE TABLE IF NOT EXISTS bad_drilling_data (
...
log.info("bad_drilling_data table is ready")
...
while True:
    try:
        ...
        log.info(f"Inserted bad record: {clean_data['sequence_number']}")
        print("Inserted bad record:", clean_data["sequence_number"])
